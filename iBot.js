...
(function(){
    ...
    function ChatPM(user, msg) {
        if ($('#chat-button').css('display') == 'block') {
            ...
            chat.append('<div class="cm tastyplug-pm' + k + '"><div class="badge-box"><i class="icon icon-ignored"></i></div><div class="msg"><div class="from pm">'+ header+'<span class="un clickable">' + user + ' </span><span class="timestamp" style="display: inline;">' + f + '</span></div><div class="text">' + msg + '</div></div></div>');
            if (a) chat.scrollTop(chat[0].scrollHeight);
            if (chat.children().length >= 512) chat.children().first().remove();
    ...
    function custEmotes(a) {
        ...
        var c = html.toLowerCase(), chat = $('#chat-messages'), d = chat.scrollTop() > chat[0].scrollHeight - chat.height() - 28;
        for (var i in emotes) {
            if (emotes[i].hidden && !settings.hidden) continue;
            var j = ':' + i.toLowerCase() + ':';
            html = html.replace(new RegExp(j,'gi'),'<div class="custom-emote" title="' + (emotes[i].hidden ? 'Hidden Emote!' : i) + '" style="background-image:url(' + emotes[i].url + ');width:' + emotes[i].width + ';height:' + emotes[i].height + ';"></div>');
        }
        a.html(html);
        if (d) chat.scrollTop(chat[0].scrollHeight);
    }
    ...
    var legacyChat = (function() {
        "use strict";
        var legacyChatStylesheet = "div#chat-messages div.cm.legacy-chat{min-height:0;width:auto}div#chat div.cm.legacy-chat.mention{background:#0a0a0a}div#chat div.cm.legacy-chat.mention:nth-child(2n+1){background:#111317}div#chat-messages div.legacy-chat div.badge-box{height:0;overflow:hidden;position:absolute}#chat-messages div.from.pm span.pm-header{padding:0;color:#F59425}div.legacy-chat i.icon{margin:6px}div.legacy-chat i.icon.icon-ignored{margin:0}#chat-messages div.legacy-chat.is-admin{border-left:3px solid #42a5dc}#chat-messages div.legacy-chat.is-ambassador{border-left:3px solid #89be6c}#chat-messages div.legacy-chat.is-staff,#chat-messages div.legacy-chat.is-dj{border-left:3px solid #ac76ff}#chat-messages div.legacy-chat.is-you{border-left:3px solid #ffdd6f}#chat-messages div.legacy-chat.is-pm{border-left:3px solid #F59425}#chat-messages div.legacy-chat div.msg{padding:4px 5px 5px 27px}div.legacy-chat div.msg div.from span.un{padding-right:3px}#chat-messages div.text{display:inline}#chat-messages div.text:before{content:' '}div.legacy-chat.mention div.msg div.from div.text,div.legacy-chat.message div.msg div.from div.text{color:#eee}div.legacy-chat div.msg div.from:before{content:'';float:right;width:50px;height:1px}#chat-messages div.legacy-chat div.msg div.from span.timestamp{position:absolute;right:0;top:1px;float:right}.tastyplug-message.legacy-chat span.un{display:none}#chat-messages div.mention.legacy-chat i.icon,#chat-messages div.mention.legacy-chat i.icon.icon-ignored{left:-3px}#chat-messages div.mention div.msg{padding-left:24px}#chat-messages .legacy-chat .delete-button{padding:1px 7px}",
        ...
        var convertToLegacy = function(node) {
            ...
            if(icon.length) {
                ...
                icon = $('<i class="icon icon-ignored legacy-fake-icon"></i>');
                icon.insertAfter(badge);
